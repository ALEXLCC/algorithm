...
/*		if(players[cur_player].miss){
	...
	*/
		...
		for(j=0;j<roll_count;++j){
			if(cur_pos==ladderchute[j].start){
				cur_pos=ladderchute[j].end;
			}
		}
...
int main(void){
	...
	for(i=0;i<player_count;++i){
		...
		while(scanf("%d%d",&ladderchute[i].start,&ladderchute[i].end)){
			if(0==ladderchute[i].start||0==ladderchute[i].end){
				break;
			}
			++i;
		}
